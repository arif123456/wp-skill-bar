...
function wpskill_settings_save_meta($post_id){
    if (isset($_POST['wpskill_settings_save_meta_data_action'])) {
        ...
        update_post_meta($post_id, 'skill_title_color', esc_attr( $skill_title_color ) );
        update_post_meta($post_id, 'skill_bar_bg_color', esc_attr( $skill_bar_bg_color ) );
        update_post_meta($post_id, 'skill_bar_percentage_bg_color', esc_attr( $skill_bar_percentage_bg_color ) );
        update_post_meta($post_id, 'skill_value_bg_color', esc_attr( $skill_value_bg_color ) );

        update_post_meta($post_id, 'skill_title_font_size', esc_attr( $skill_title_font_size ) );
        update_post_meta($post_id, 'skill_value_font_size', esc_attr( $skill_value_font_size ) );

       
        update_post_meta($post_id, 'skill_title_font_family', esc_attr( $skill_title_font_family ) );
        update_post_meta($post_id, 'pValue_font_family', esc_attr( $pValue_font_family ) );

        update_post_meta($post_id, 'skill_bar_height', esc_attr( $skill_bar_height ) );
        update_post_meta($post_id, 'skill_bar_b_radius', esc_attr( $skill_bar_b_radius ) );
    }
}
